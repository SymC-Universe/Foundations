...
**Checkpoint ID:** D02B-CP9-ONE-COMMAND-IMPLEMENTED-PREEXECUTION  
**Date:** 2026-09-23  
...
## D02B implementation state

The raw parser/analyzer, regression tests, one-command reviewer entrypoint, and dedicated archival workflow are implemented.

Reviewer command:

`python chi_architecture/reproduce.py d02b`

The full run has not yet been interpreted. The runner deliberately leaves the added-value verdict unassigned.

## Resume rule

1. Observe the dedicated D02B workflow generated by this implementation commit.
2. Mechanical failures may be repaired without changing CP5/CP8 science.
3. On success, archive run ID, artifact ID/digest, result hash, manifest hash, and source hashes before interpretation.
4. Only then assign the scientific/native-toolkit verdict.
